...
def opa(a, b):
    ...
    # Rotate mutable B onto fixed A (svd helps but the method for optimal rotation matrix U is Kabsch's)
    B_ = B.dot(aR)
...
def avg(v):
    ...
    # Make a shape collection copy (we don't want to overwrite the original data) while making sure it's a well-formatted one (numpy)
    v = np.copy(v)
    ...
    for i in range(l):
        ...
        r = full_3d_rotation_matrix(a)  # Apply full 3D rotation matrix
...
# Function for generating a full 3D rotation matrix (rotation around X, Y, Z axes)
def full_3d_rotation_matrix(a):
    ...
    # Combine the rotations to apply them
